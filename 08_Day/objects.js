...
dog.breed = false;
dog.getDogInfo = `Dog\'s name is ${dog.name} and he is ${dog.age} years old.\nHe has ${dog.legs} legs and his color is ${dog.color}.`
...
function bbb(){
    ...
    let b = [];
    for(let i = 0;i < keys.length;i++){
        if(entries[i].isLoggedIn.valueOf()){
            a.push(entries[i]);
        };

        if(entries[i].points.valueOf() >= 50){
            b.push(entries[i]);
        }
    };
