...
let cs = ["Htet",'Paing','Kyaw','Win','War'];
...
let F = new Set(f);
let G = new Set(g);
let H = new Set(h.sort());

console.log(H);

let i = f.filter(el => G.has(el));

console.log(i);

let j = f.filter(el => !(G.has(el)));
...
let o = `There are ${p.length} languages in the countries object file.`
...
for(q of qs){
    ...
    r.push({languages : q,count : z.length });
};
...
function mostSpokenLanguages(b){
    for(let i = 0;i < b;i++){
        x = 0;
        for(ra of r){
            if(ra.count > x){
